...
func QuadB(x, y int) { // 1st line
	if x >= 1 && y >= 1 {
		z01.PrintRune('/')                             // top left corner / starts on first column
		for xcounter := 2; xcounter <= x; xcounter++ { // loop (starts on the 2nd column and ends equal to or lessthan x)
			if xcounter == x { // conditions for the loop (when xcounter is equal to x it will print '\', if not then print '*')
				z01.PrintRune('\\') // top right corner
			} else {
	...
	if y > 1 { // when y is greater than 1 print 'o'
		z01.PrintRune('\\')                            // bottom left corner
		for xcounter := 2; xcounter <= x; xcounter++ { // loop (starts on the 2nd column and ends equal to or lessthan x)
			if xcounter == x { // conditions for the loop (when xcounter is equal to x it will print '*', if not then print '/')
				z01.PrintRune('/') // bottom right corner
			} else {
				z01.PrintRune('*') // in the middle
			}
		}
